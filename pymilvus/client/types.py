...
class _ConsistencyLevel:
    ...
    _values = {0, 1, 2, 3, 4}
    _names = {"Strong": 0, "Session": 1, "Bounded": 2, "Eventually": 3, "Customized": 4}
...
class DataType(IntEnum):
    ...
    _ARRAY_OF_VECTOR = 106
    _ARRAY_OF_STRUCT = 200
...
class GrantInfo:
    ...
    def __init__(self, entities: List[RoleEntity]) -> None:
        groups = []
        for entity in entities:
            if isinstance(entity, GrantEntity):
...
class PrivilegeGroupItem:
    def __init__(self, privilege_group: str, privileges: List[PrivilegeEntity]):
        self._privilege_group = privilege_group
        privielges = []
        for privilege in privileges:
            if isinstance(privilege, PrivilegeEntity):
...
class PrivilegeGroupInfo:
    ...
    def __init__(self, results: List[PrivilegeGroupInfo]) -> None:
        groups = []
        for result in results:
            if isinstance(result, PrivilegeGroupInfo):
...
class UserItem:
    def __init__(self, username: str, entities: List[RoleEntity]) -> None:
        self._username = username
        roles = []
        for entity in entities:
            if isinstance(entity, RoleEntity):
...
class UserInfo:
    ...
    def __init__(self, results: List[UserResult]):
        groups = []
        for result in results:
            if isinstance(result, UserResult):
...
class RoleItem:
    def __init__(self, role_name: str, entities: List[UserEntity]):
...
class RoleInfo:
    ...
    def __init__(self, results: List[RoleResult]) -> None:
        groups = []
        for result in results:
            if isinstance(result, RoleResult):
...
def get_cost_from_status(status: Optional[Status] = None):
    return int(
...
def get_extra_info(status: Optional[Status] = None):
...
class AnalyzeToken:
    def __init__(
        self, token: Optional["AnalyzerToken"] = None, with_hash: bool = False, with_detail: bool = False
    ):
...
class AnalyzeResult:
    def __init__(
        self, info: Optional["AnalyzerResult"] = None, with_hash: bool = False, with_detail: bool = False
    ) -> None:
...
class FileResourceInfo:
    def __init__(self, info: FileResourceInfo) -> None:
        self.name = info.name
        self.path = info.path
...
@dataclass
class ResourceGroupLimit:
    node_num: int = 0


@dataclass
class ResourceGroupTransfer:
    resource_group: str = ""


@dataclass
class ResourceGroupConfig:
    requests: Optional[ResourceGroupLimit] = None
    limits: Optional[ResourceGroupLimit] = None
    transfer_from: Optional[List[ResourceGroupTransfer]] = None
    transfer_to: Optional[List[ResourceGroupTransfer]] = None


class HybridExtraList(list):
    def __init__(self, lazy_fields, results, extra=None, dynamic_fields=None, strict_float32=False, element_indices=None):
        super().__init__(results if results else [])
        self.extra = extra or {}
        self.dynamic_fields = dynamic_fields
        self.strict_float32 = strict_float32
